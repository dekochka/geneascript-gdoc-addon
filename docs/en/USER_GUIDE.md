...
At the bottom of the dialog, a **Show Usage Stats in sidebar** checkbox lets you opt in to the Usage Stats panel (see below). This setting is OFF by default and saved per user account.
...
The sidebar can display a collapsible **Usage Stats** panel that shows token consumption and estimated cost for your transcription work. This is useful for monitoring API spending and understanding how template choice and image size affect cost.

![Usage Stats sidebar section](app-screenshots/v1.6.1-en-UsageStats-section-feature.jpg)

### Enabling Usage Stats

Usage Stats is **off by default**. To enable it:

1. Open **Setup AI** (sidebar button or menu).
2. Check **Show Usage Stats in sidebar** at the bottom of the dialog.
3. Click **Save**. The stats panel appears in the sidebar within a few seconds.

### What the panel shows

The panel has two sections: **Document total** (cumulative across all batches) and **Last run** (most recent batch only).

| Metric | Description |
|--------|-------------|
| **Model** | The Gemini model used for transcription (e.g. `gemini-2.5-flash`) |
| **Images transcribed** | Number of successfully transcribed images |
| **Input tokens** | Total tokens sent to the API (prompt + image data) |
| ↳ prompt (text) | Text portion of input tokens (template/prompt) — shown when the API provides a breakdown |
| ↳ image | Image portion of input tokens (scan data) — shown when the API provides a breakdown |
| **Output tokens** | Tokens generated by the model (the transcription text) |
| **Thinking tokens** | Internal reasoning tokens — only shown when > 0 (thinking-enabled models) |
| **Total tokens** | Sum of input + output + thinking tokens |
| **Est. cost** | Estimated cost in USD based on the model’s per-token pricing |
| **Avg / image** | Average tokens and cost per image — shown when images > 0 |

### Footer links

- **Rates** — opens the [Gemini API pricing](https://ai.google.dev/pricing) page
- **My Spend** — opens your [AI Studio spend dashboard](https://aistudio.google.com/spend)
- **Reset** — clears all document stats to zero (with confirmation)

### How stats work

- Stats **accumulate across multiple batch runs** within the same document — start a second batch and totals keep growing.
- Stats are **persisted per document** (saved to your user account, tied to the document ID). Reloading the sidebar or reopening the document restores your stats.
- **Last run** section appears after the first batch completes and shows only the most recent batch.
- **Reset** clears all stats (document total, last run) and deletes the persisted data.
- Failed images do not accumulate tokens — only successful transcriptions count.
- Cost estimation uses hardcoded per-model pricing rates. Thinking tokens are not billed and excluded from cost calculation.
