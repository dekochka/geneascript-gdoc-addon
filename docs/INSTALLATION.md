...
flowchart LR
  ...
  O1 --> Key1[Set GEMINI_API_KEY]
  O2 --> Key2[Set GEMINI_API_KEY]
  O3 --> Key3[Set GEMINI_API_KEY]
...
6. **🔑** Set the API key: **Project Settings** → **Script properties** → add `GEMINI_API_KEY` with your key.
...
6. **▶️** Run **onOpen** once from the script editor (authorize if prompted). **🔄 Reload** the document; the menu **Extensions** → **Metric Book Transcriber** → **Transcribe Image** should appear. Running **Transcribe Image** opens a dialog (“Awaiting response from Gemini API… This may take up to 1 minute”) and inserts the result below the selected image.
