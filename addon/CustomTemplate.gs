...
function getCustomTemplateEditorHtml(templateId) {
  ...
  return [
    ...
    '.section-editor .sec-ta { width: 100%; box-sizing: border-box; min-height: 180px; padding: 8px 10px; border: none; font-family: "Roboto Mono", "Courier New", monospace; font-size: 11px; line-height: 1.5; color: #333; resize: vertical; flex: 1; }',
    '.section-editor .sec-ta:focus { outline: none; background: #fafbff; }',
    ...
    '.confirm-box .btn-confirm-danger { padding: 7px 18px; border-radius: 4px; font-size: 13px; cursor: pointer; color: #fff; background: #c5221f; border: 1px solid #c5221f; }',
    '.confirm-box .btn-confirm-danger:hover { background: #a81e1b; }',
    '</style>',
    '</head><body>',
    ...
    '    <textarea id="sec_role" class="sec-ta"></textarea>',
    '    <textarea id="sec_inputStructure" class="sec-ta" style="display:none"></textarea>',
    '    <textarea id="sec_outputFormat" class="sec-ta" style="display:none"></textarea>',
    '    <textarea id="sec_instructions" class="sec-ta" style="display:none"></textarea>',
    '    <textarea id="sec_contextDefaults" class="sec-ta" style="display:none"></textarea>',
    '  </div>',
    ...
    '      <button class="btn" onclick="closeConfirm()">' + esc(t('editor.cancel')) + '</button>',
    '      <button class="btn-confirm-danger" id="confirmYes"></button>',
    '    </div>',
    '  </div>',
    '</div>',
    '<script>',
    ...
    'var currentTab="role";',
    'var SEC_KEYS=["role","inputStructure","outputFormat","instructions","contextDefaults"];',
    ...
    '',
    'document.getElementById("sec_role").value = tpl.sections ? tpl.sections.role || "" : "";',
    'document.getElementById("sec_inputStructure").value = tpl.sections ? tpl.sections.inputStructure || "" : "";',
    'document.getElementById("sec_outputFormat").value = tpl.sections ? tpl.sections.outputFormat || "" : "";',
    'document.getElementById("sec_instructions").value = tpl.sections ? tpl.sections.instructions || "" : "";',
    'document.getElementById("sec_contextDefaults").value = tpl.contextDefaults || "";',
    '',
    ...
    '',
    'SEC_KEYS.forEach(function(k){ document.getElementById("sec_"+k).addEventListener("input", function(){ document.getElementById("emptyHint").style.display = this.value.trim() ? "none" : "block"; }); });',
    'showSection("role");',
    '',
    ...
    '  SEC_KEYS.forEach(function(k){ document.getElementById("sec_"+k).style.display = k===key ? "" : "none"; });',
    '  var ta = document.getElementById("sec_"+key);',
    '  document.getElementById("emptyHint").style.display = ta.value.trim() ? "none" : "block";',
    '}',
    '',
    'function doReset(){',
    '  if(!parentId) return;',
    '  showConfirm(EI.resetConfirm, EI.resetBtn, function(){',
    '    if(parentSections){',
    '      applyReset();',
    '    } else {',
    ...
    '  document.getElementById("sec_"+currentTab).value = parentSections[currentTab] || "";',
    '  showSection(currentTab);',
    '}',
    '',
    'function doSave(){',
    ...
    '    sections: {',
    '      role: document.getElementById("sec_role").value,',
    '      inputStructure: document.getElementById("sec_inputStructure").value,',
    '      outputFormat: document.getElementById("sec_outputFormat").value,',
    '      instructions: document.getElementById("sec_instructions").value',
    '    },',
    '    contextDefaults: document.getElementById("sec_contextDefaults").value,',
    ...
    '      setTimeout(function(){ google.script.run.showTemplateGalleryDialog(); }, 1000);',
    '    } else {',
    ...
    'function doDelete(){',
    '  if(!tpl.id) return;',
    '  showConfirm("' + esc(t('gallery.confirm_delete')) + '", EI.deleteBtn, function(){',
    '    google.script.run.withSuccessHandler(function(r){',
    '      if(r.ok){ showStatus("success", r.message); google.script.run.showTemplateGalleryDialog(); }',
