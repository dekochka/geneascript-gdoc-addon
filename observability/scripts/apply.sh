...
upsert_counter_metric \
  ...
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"transcribe_image_done\".*\"status\":\"success\""'

upsert_counter_metric \
  "geneascript_import_runs_count" \
  "Count completed import runs" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"import_drive_done\".*\"status\":\"success\""'

upsert_counter_metric \
  "geneascript_errors_count" \
  "Count error events across operations" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"status\":\"error\""'

upsert_distribution_metric \
  "geneascript_prompt_tokens" \
  "Prompt tokens per image" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"transcribe_image_api_done\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"promptTokens\\\":([0-9]+).*")'

upsert_distribution_metric \
  "geneascript_output_tokens" \
  "Output tokens per image" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"transcribe_image_api_done\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"outputTokens\\\":([0-9]+).*")'

upsert_distribution_metric \
  "geneascript_total_tokens" \
  "Total tokens per image" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"transcribe_image_api_done\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"totalTokens\\\":([0-9]+).*")'

upsert_distribution_metric \
  "geneascript_transcribe_latency_ms" \
  "End-to-end transcription latency in milliseconds" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"transcribe_image_done\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"latencyMs\\\":([0-9]+).*")'

upsert_distribution_metric \
  "geneascript_image_bytes" \
  "Input image size in bytes for transcriptions" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"transcribe_image_api_done\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"imageBytes\\\":([0-9]+).*")'

upsert_distribution_metric \
  "geneascript_images_imported_count" \
  "Images imported per completed import run" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"import_drive_done\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"addedCount\\\":([0-9]+).*")'

upsert_distribution_metric \
  "geneascript_import_image_latency_ms" \
  "Per-image import latency in milliseconds" \
  'resource.type="app_script_function" AND textPayload=~"OBS:.*\"event\":\"import_drive_image_processed\".*\"status\":\"success\""' \
  'REGEXP_EXTRACT(textPayload, ".*\\\"imageImportLatencyMs\\\":([0-9]+).*")'
...
if [[ -n "${dashboard_name}" ]]; then
  ...
  gcloud monitoring dashboards update "$dashboard_name" --config-from-file="$DASHBOARD_FILE" >/dev/null
else
